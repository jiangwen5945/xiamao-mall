<template>
  <j-dialog v-model="dialogVisible" title="查看物流" width="32%" lock>
...
export default {
  setup () {
    ...
    const open = (item) => {
      ...
      // 查询物流信息
      proxy.$api.logisticsOrder(item.id).then(res => {
        logistics.value = res
      })
    }
    return { dialogVisible, open, logistics }
  }
}
</script>
